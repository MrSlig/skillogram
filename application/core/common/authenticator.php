...
/*
	...
	Description:
	    ...
	    2. The login cookie contains the user's username, a series identifier, and a token. The series and token are unguessable random numbers (or sring) from a suitably large space. All three are stored together in a database table.
		...
		[*] For the most secure result, the standard session management cookie should be a "session" cookie that expires as soon as the Web browser is closed. Furthermore, the server should enforce a fairly short maximum lifetime on sessions even if the browser remains open.
...
class	Authenticator
{
			...
			if (isset($passport['login'])) {
				...
				if ($passport['token']	==	$_COOKIE['token'])	{
					...
					if ($passport['serial']	==	$_COOKIE['serial'])	{
						...
						// FUBAR!; report user last entery date: smbd probably messed with ther cookies (read: account)!
	...
	public static function	createCookie($dbh, $login)
	{
		...
		setcookie('login',	$passport['login'],		$passport['date'] + MONTH, '/');	//range '/' = whole site
